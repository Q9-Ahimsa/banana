...
/**
 * Parse the argv slice after the `brief` subcommand.
 * @param {string[]} argv
 * @returns {BriefFlags}
 * @throws on missing feature/tag or unknown options
 */
export function parseBriefArgs(argv) {
  ...
  if (!feature) throw new Error('a feature slug is required (banana brief <feature> --tag <agent>)');
  if (!tag) throw new Error('--tag <agent> is required (who is reading this brief)');
  return { feature, tag };
}
...
export function compileBrief(flags, deps) {
  ...
  const logRef = '.agents/session.log';
  const logPath = join(cwd, '.agents', 'session.log');
  if (!existsSync(logPath)) {
    throw new Error(`no ${logRef} here — run \`banana project\` to initialize this repo`);
  }
...
/**
 * Run the brief command: compile and print to stdout.
 * @param {BriefFlags} flags
...
export async function runBrief(flags, deps) {
  ...
  try {
    io.out(compileBrief(flags, { cwd, now }));
    return { code: 0 };
  } catch (error) {
